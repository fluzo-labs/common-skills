...
| 7. Continuar o terminar | «Usa delivery-review-github para verificar el merge de [URL de PR] y recomendar la siguiente issue, con título, enlace y motivo. Todavía no la implementes». Se prioriza la siguiente fase elegible del plan actual; tras completarlo, se consulta solo el alcance autorizado del backlog. |
...
| Preparar entrega | [delivery-review-github](skills/delivery-review-github/SKILL.md) | Propone push explícito y PR con evidencia; publica solo lo autorizado. En una comprobación solicitada tras el merge, recomienda la siguiente issue elegible sin empezarla. |
...
Refina una issue, aprueba su plan, ejecuta una fase elegible, actualiza el progreso, elige un commit, publica la rama revisada, prepara su PR y regresa tras el merge para la siguiente issue. Cada skill termina con el resultado real y **un siguiente paso recomendado**, su motivo y una respuesta breve ligada a la issue, fase, rama o documento real. Primero se resuelven bloqueos; «revisar» o «detenerse» son alternativas secundarias, no un menú genérico. Las recomendaciones respetan reglas superiores de respuesta, incluida la respuesta vacía, y nunca se ejecutan solas. Un acuerdo confirmado puede documentarse con `convention-document` en cualquier momento, conservando la fase de origen y el paso pendiente para un retorno concreto.

Después del commit se recomienda preparar push y PR para la rama revisada, o verificar/actualizar su PR existente. La skill de commits sigue siendo local y no consulta GitHub ni hace push; entrega verifica destino, commits salientes, SHA remoto y PR existente antes de publicar. «Preparar push y PR» solo autoriza propuestas. Push, creación/edición de PR y cualquier comentario de issue necesitan autorización explícita de las operaciones y contenido exactos, que pueden aprobarse conjuntamente. No se repiten aprobaciones sin cambios ni se duplican PRs; cambios de alcance o revisión requieren revalidación. El trabajo solo local conserva una ruta de revisión local sin exigir GitHub.

Después de publicar la PR, la recomendación sigue el estado real de checks y revisión; el merge sigue siendo una operación del consumidor autorizada por separado, no una acción de la skill de entrega. Ante «la PR está mergeada, ¿qué sigue?», `delivery-review-github` verifica merge, destino, aceptación y cierre real de issues antes de recomendar una issue existente con número cualificado, título, enlace y motivo. Prioriza la fase elegible del plan actual; tras completarlo, usa prioridades y dependencias descubiertas dentro del backlog autorizado. Recomienda refinamiento para trabajo poco claro, no ejecución prematura. Se explicitan falta de acceso, cierres pendientes, fases bloqueadas, prioridades empatadas y backlog agotado. No se asigna trabajo, cambia estado, crea rama ni implementa automáticamente; no hay observador. Una PR de release vuelve a actualizar evidencias, nunca a publicar automáticamente.
