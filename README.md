...
| 7. Continue or finish | "Use delivery-review-github to verify the merge of [PR URL] and recommend the next issue, with its title, link, and reason. Do not implement it yet." The current plan's next eligible phase takes priority; once the plan is complete, inspect only the authorized backlog scope. |
...
| Prepare delivery | [delivery-review-github](skills/delivery-review-github/SKILL.md) | Proposes an explicit push and PR with evidence; publishes only what is authorized. On a requested post-merge check, recommends the next eligible issue without starting it. |
...
Refine an issue, approve its plan, execute one eligible phase, update progress, choose a commit, publish the reviewed branch, prepare its PR, and return after merge for the next issue. Each skill ends with the actual result and **one recommended next step**, its reason, and a short reply tied to the real issue, phase, branch, or document. Resolve blockers before advancing; keep "revise" or "stop" as secondary alternatives, not a generic menu. Recommendations respect higher-priority response rules, including empty responses, and never execute themselves. A confirmed convention can be documented with `convention-document` at any point, retaining the originating phase and pending gate for a concrete return.

After a commit, the recommendation is to prepare push and PR for the reviewed branch, or verify/update its existing PR. The commit skill remains local and does not query GitHub or push; delivery verifies the destination, outgoing commits, remote SHA, and existing PR before publication. A reply such as "prepare push and PR" authorizes proposals only. Push, PR creation/editing, and any issue comment need explicit authorization of the exact operations and content, which can be approved together. Do not repeat unchanged approvals or duplicate PRs; changed scope or revisions require revalidation. Local-only work keeps a local review path without requiring GitHub.

After PR publication, the recommendation follows actual checks and review state; merge remains a separately authorized consumer operation, not an action of the delivery skill. On "the PR is merged, what next?", `delivery-review-github` verifies merge, target, acceptance, and actual issue closure before recommending one existing issue with its qualified number, title, link, and rationale. Prefer the current plan's eligible phase; after completion, use discovered priorities and dependencies within the authorized backlog. Recommend refinement for unclear work, not premature execution. Missing access, unresolved closure, blocked phases, tied priorities, and an exhausted backlog are reported rather than hidden. No assignment, state change, branch creation, or implementation follows automatically; there is no watcher. A release PR returns to refreshed release evidence, never automatic publication.
